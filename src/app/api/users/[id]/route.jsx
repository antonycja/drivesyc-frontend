...
function handleApiError(status, data, defaultMessage) {
    const errorMap = {
        ...
        404: { message: 'User not found.', code: 'NOT_FOUND' },
...
export async function GET(request, { params }) {
    try {
        ...
        const { data, status } = await serverFetch(`${API_BASE}/users/${id}`);
        if (status === 200) return NextResponse.json(data, { status: 200 });
        return handleApiError(status, data, 'Failed to fetch user');
    } catch (error) {
...
export async function PATCH(request, { params }) {
    try {
        ...
        if (status === 200) return NextResponse.json(data, { status: 200 });
        return handleApiError(status, data, 'Failed to update user');
    } catch (error) {
