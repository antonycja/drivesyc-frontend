...
function handleApiError(status, data, defaultMessage) {
    ...
    const error = errorMap[status] || { message: defaultMessage || 'Something went wrong.', code: 'UNKNOWN_ERROR' };
...
export async function PATCH(request, { params }) {
    try {
        ...
        const { data, status } = await serverFetch(`${API_BASE}/bookings/${id}/complete`, {
            method: 'PATCH',
        });
        if (status === 200) return NextResponse.json(data, { status: 200 });
        return handleApiError(status, data, 'Failed to complete booking');
    } catch (error) {
